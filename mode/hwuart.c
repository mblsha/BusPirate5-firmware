 ...
 */
	...
	// uart_menu[4] is blocking, skip for now
...
uint32_t hwuart_setup_exc(void){
	...
	/* uart_set_fifo_enabled(M_UART_PORT, true); // needed for flow control, true */
	/* 																					// is default */
	...
	// https://github.com/argilo/picoprobe/commit/50affa6517e53f5299deb69cce0715d4c784c45c#diff-39bcb9935a8cacb67fbf811e42802e892b0a385df457ddb8012d044a01306319R56
...
void hwuart_write(struct _bytecode *result, struct _bytecode *next){
	if(mode_config.blocking){
		uart_putc(M_UART_PORT, result->out_data);
	}else{
		uart_putc(M_UART_PORT, result->out_data);
	}
}
